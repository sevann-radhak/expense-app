...
**Phase 5 status:** `in progress` (**5.1** done; **5.2** local environment — **in progress**).
...
|------|--------|------------|-----|
| **Local** | Developer PC | **$0** | Day-to-day implementation: Drift; **`expense-app-backend`** `ExpenseTracker.Api` on localhost (default **http://localhost:5057**); optional Docker SQL Server. |
...
| 5.2      | **Local** dev environment: tooling, SQL optional, API skeleton, checklist | `in progress` |
...
- [ ] **.NET SDK** installed; in **`expense-app-backend`**: `dotnet build ExpenseTracker.sln -c Release` succeeds.
- [ ] **API run:** `dotnet run --project ExpenseTracker.Api` → **Swagger UI** at `/swagger` loads; **`GET /api/health`** returns JSON `status` **ok**; **`GET /api/hello`** returns `message` (see that repo’s `README.md`).
- [ ] **Flutter:** `flutter doctor` actionable items addressed; `flutter pub get`, `flutter analyze`, `flutter test` green.
- [ ] **Optional SQL:** Docker (or LocalDB) runs **SQL Server**; empty database created; **connection string** documented in a **gitignored** local file or env in **`expense-app-backend`** (`README.md` or `.env.example` — **never** commit secrets).
- [ ] **Optional:** Install **Azure Functions Core Tools** if you later migrate the host to Functions (`dotnet new func` requires the Functions worker template).
- [ ] **Docs:** This checklist is **checked off** in a PR when 5.2 closes.
