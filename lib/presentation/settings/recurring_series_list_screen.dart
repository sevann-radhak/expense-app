...
import 'package:expense_app/presentation/settings/recurring_income_series_edit_dialog.dart';
...
class RecurringSeriesListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(l10n.recurringHubAppBarTitle),
          bottom: TabBar(
            isScrollable: true,
            tabAlignment: TabAlignment.start,
            tabs: [
              Tab(text: l10n.recurringSeriesTabExpenses),
              Tab(text: l10n.recurringSeriesTabIncome),
            ],
          ),
        ),
        body: const TabBarView(
          children: [
            _RecurringExpenseSeriesTab(),
            _RecurringIncomeSeriesTab(),
          ],
        ),
      ),
    );
...
class _RecurringExpenseSeriesTab extends ConsumerWidget {
  const _RecurringExpenseSeriesTab();
...
class _RecurringIncomeSeriesTab extends ConsumerWidget {
  const _RecurringIncomeSeriesTab();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = AppLocalizations.of(context)!;
    final async = ref.watch(recurringIncomeSeriesListProvider);
    return async.when(
      data: (list) {
        if (list.isEmpty) {
          return Padding(
            ...
            child: Center(
              child: Text(
                l10n.recurringSeriesIncomeEmpty,
                textAlign: TextAlign.center,
                style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                      color: Theme.of(context).colorScheme.onSurfaceVariant,
                    ),
        ...
        return ListView.separated(
          ...
          itemBuilder: (ctx, i) {
            final s = list[i];
            final title = s.description.trim().isEmpty
                ? l10n.recurringSeriesDefaultTitleIncome
                : s.description.trim();
            final subtitle =
                '${recurrenceRuleSummary(context, s.rule)}\n${l10n.recurringSeriesHorizonMonths(s.horizonMonths)} · ${s.active ? l10n.recurringSeriesActive : l10n.recurringSeriesInactive}';
            return Card(
              child: ListTile(
                ...
                onTap: () {
                  showDialog<void>(
                    context: context,
                    builder: (c) => RecurringIncomeSeriesEditDialog(series: s),
                  );
                },
                trailing: s.active
                    ? PopupMenuButton<_SeriesMenuAction>(
                        onSelected: (action) async {
                          if (action == _SeriesMenuAction.stop) {
                            ...
                            if (ok == true && context.mounted) {
                              await ref
                                  .read(
                                    recurringIncomeSeriesRepositoryProvider,
                                  )
                                  .deactivateSeries(
                                    seriesId: s.id,
                                    todayDateOnly: calendarTodayLocal(),
